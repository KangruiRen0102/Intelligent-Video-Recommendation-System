...
line=$(wc -l deployments | cut -d " " -f 1)
sed -i -e "${line}s/false/true/" "${filepath}/deployments.csv"

# Find the last working version
repo="prod_fastapi_server"
version=$(grep false deployments.csv | tail -n 1 | cut -d "," -f 3)
...
# Remove possibly existing containers
remove 7003 7005
